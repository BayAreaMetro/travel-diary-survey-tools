...
if __name__ == "__main__":
    ...
    output_lines.append("")
    output_lines.append(f"Households in legacy: {len(legacy_hhnos):,}")
    output_lines.append(f"Households in new: {len(new_hhnos):,}")
    output_lines.append(f"Households in both: {len(common_hhnos):,}")
    ...
    if len(common_hhnos) >= NUM_SAMPLE_HOUSEHOLDS:
        ...
        output_lines.append("")
        output_lines.append(f"Sampling households: {sample_hhnos}")
        logger.info("\n".join(output_lines))
        compare_household_diaries(sample_hhnos, legacy, new)
    else:
        output_lines.append("")
        output_lines.append("Not enough common households to sample.")
        logger.info("\n".join(output_lines))
