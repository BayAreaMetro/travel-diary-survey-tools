...
trips_23 = pl.read_csv(persons_23_dir / "trip.csv")
days_23 = pl.read_csv(persons_23_dir / "day.csv")

# Load and apply revised 2023 day weights
day_weights_23 = pl.read_csv(weights_23_dir / "day_weights.csv")
days_23 = (
...
| 1-3 days a month       | `telework_freq` == 7 AND `job_type` != 3    | `commute_freq` == 6 AND `telework_freq` != 8 AND `job_type` != 3    |
| Less than monthly      | `telework_freq` == 6 AND `job_type` != 3    | `commute_freq` == 7 AND `telework_freq` != 8 AND `job_type` != 3    |
| Never                  | `telework_freq` == 8 AND `job_type` != 3    | `commute_freq` == 8 OR `job_type` == 3                              |
...
telework_conditions = {
    ...
    FrequencyCategory.NEVER: (
        (pl.col("telework_freq") == 8) & (pl.col("job_type") != 3)
    ),
    # Missing
...
flavia_comparison_df = (
    ...
    .select(["telework_freq_str", "Nick's % (wtd)"])
