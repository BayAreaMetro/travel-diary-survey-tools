...
def _compute_jtf_choice(
    ...
    Args:
        ...
        tours_canonical: Canonical tours DataFrame with tour_purpose and joint_tour_id
    ...
    # Map tour purposes to JTF categories
    jtf_categories = {
        "S": [  # Shopping
            Purpose.ROUTINE_SHOPPING,
            Purpose.MAJOR_SHOPPING,
            Purpose.GROCERY,
            Purpose.SHOPPING_ERRANDS,
            Purpose.GAS,
        ],
        "M": [  # Maintenance
            Purpose.MEDICAL,
            Purpose.ERRAND_NO_APPT,
            Purpose.ERRAND_WITH_APPT,
            Purpose.OTHER_ERRAND,
        ],
        "E": [  # Eating out
            Purpose.DINING,
        ],
        "V": [  # Visiting
            Purpose.SOCIAL,
            Purpose.SOCIAL_LEISURE,
            Purpose.OTHER_SOCIAL,
        ],
        "D": [  # Discretionary (and all other purposes for completeness)
            Purpose.ENTERTAINMENT,
            Purpose.EXERCISE,
            Purpose.RELIGIOUS_CIVIC,
            Purpose.FAMILY_ACTIVITY,
            # Work/School (typically not joint)
            Purpose.PRIMARY_WORKPLACE,
            Purpose.WORK_ACTIVITY,
            Purpose.WORK_VOLUNTEER,
            ...
            Purpose.DROP_OFF,
            Purpose.ACCOMPANY,
            Purpose.PICK_UP_AND_DROP_OFF,
            Purpose.OTHER_ACTIVITY,
            Purpose.OTHER,
            # Home/Lodging
            Purpose.HOME,
            Purpose.OTHER_RESIDENCE,
            Purpose.TEMP_LODGING,
            # Mode change/Missing
            Purpose.MODE_CHANGE,
            Purpose.MISSING,
            Purpose.PNTA,
            Purpose.NOT_IMPUTABLE,
        ],
    }

    # Invert to create purpose value -> category mapping
    purpose_to_jtf = {
        purpose.value: category
        for category, purposes in jtf_categories.items()
        for purpose in purposes
    }

    # Validate no duplicate purposes across categories
    all_purposes = [purpose for purposes in jtf_categories.values() for purpose in purposes]
    if len(all_purposes) != len(set(all_purposes)):
        msg = "Duplicate purposes found in JTF category mapping"
        raise ValueError(msg)

    # Tag each joint tour with its JTF category
    joint_tours = joint_tours.with_columns(
        pl.col("tour_purpose").replace_strict(purpose_to_jtf).alias("_jtf_grouping")
    )

    # Aggregate by household: count tours by category
    hh_jtf_agg = joint_tours.group_by("hh_id").agg(
        [
            (pl.col("_jtf_grouping") == "S").sum().alias("shop_count"),
            (pl.col("_jtf_grouping") == "M").sum().alias("maint_count"),
            (pl.col("_jtf_grouping") == "E").sum().alias("eatout_count"),
            (pl.col("_jtf_grouping") == "V").sum().alias("visit_count"),
            (pl.col("_jtf_grouping") == "D").sum().alias("discr_count"),
        ]
    )
