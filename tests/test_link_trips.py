...
from processing.steps.link import (
...
class TestLegacyComparison:
    """Tests comparing new implementation to legacy behavior."""

    def test_basic_change_mode_linking_matches_legacy(self):
        """New linking should match legacy for basic change_mode case."""
        # This simulates the core legacy behavior:
        # - dpurp=10 (change_mode) causes linking
        # - Next trip destination becomes the linked trip destination
        trips = pl.DataFrame({
            "trip_id": [1, 2],
            "day_id": [1, 1],
            "person_id": [100, 100],
        ...
        result = link_trips(
            ...
            change_mode_code=11,
            transit_mode_codes=[6, 7],
            max_dwell_time=35,  # Legacy default
            dwell_buffer_distance=100,
        )

        linked = result["linked_trips"]

        # Legacy behavior: merge into one trip
        assert len(linked) == 1

        row = linked.row(0, named=True)
        # Should use first trip origin
        assert row["o_purpose_category"] == 1
        # Should use last trip destination
        assert row["d_purpose_category"] == 2
        # Should use transit mode (higher hierarchy)
        assert row["mode_type"] == 6

    def test_mode_hierarchy_matches_legacy(self):
        """Mode selection follows legacy hierarchy.
        ...
        trips = pl.DataFrame({
            ...
            "arrive_minute": [10, 40, 0],
            "arrive_seconds": [0, 0, 0],
            "o_purpose_category": [1, 11, 11],
            "d_purpose_category": [11, 11, 2],
            "o_lat": [37.7, 37.71, 37.75],
            "o_lon": [-122.4, -122.41, -122.45],
            "d_lat": [37.71, 37.75, 37.8],
            "d_lon": [-122.41, -122.45, -122.5],
            "mode_type": [1, 6, 3],  # walk, transit, drive
            "distance_miles": [0.5, 10.0, 5.0],
            "duration_minutes": [10.0, 25.0, 15.0],
            "trip_weight": [1.0, 1.0, 1.0],
        })
        ...
        # Transit should be selected (mode 6)
        assert linked["mode_type"][0] == 6

    def test_activity_duration_threshold_behavior(self):
        """Should not link when activity duration exceeds threshold."""
        trips = pl.DataFrame({
            "trip_id": [1, 2],
            "day_id": [1, 1],
            "person_id": [100, 100],
            ...
            "depart_time": [
                datetime(2024, 1, 1, 8, 0),
                datetime(2024, 1, 1, 9, 0),  # 50 min gap (> 35 min)
            ],
            "arrive_time": [
                datetime(2024, 1, 1, 8, 10),
                datetime(2024, 1, 1, 9, 30),
        ...
        result = link_trips(
            ...
            change_mode_code=10,
            transit_mode_codes=[6, 7],
            max_dwell_time=35,  # Legacy threshold
            dwell_buffer_distance=100,
        )

        linked = result["linked_trips"]

        # Should NOT link (stays as 2 separate trips)
        assert len(linked) == 2

    def test_person_isolation_matches_legacy(self):
        """Different persons should never have trips linked together."""
        trips = pl.DataFrame({
            "trip_id": [1, 2],
            "day_id": [1, 1],
            "person_id": [100, 200],  # Different persons
            "hh_id": [10, 20],
            "depart_time": [
                datetime(2024, 1, 1, 8, 0),
                datetime(2024, 1, 1, 8, 5),
            ...
            "d_purpose_category": [11, 11],
            "o_lat": [37.7, 37.7],
            "o_lon": [-122.4, -122.4],
            "d_lat": [37.71, 37.71],
            "d_lon": [-122.41, -122.41],
            "mode_type": [1, 1],
            "distance_miles": [0.5, 0.5],
            "duration_minutes": [10.0, 10.0],
            "trip_weight": [1.0, 1.0],
        })
        ...
        # Each person should have unique linked_trip_id
        person1_id = trips_with_ids.filter(pl.col("person_id") == 100)[
            "linked_trip_id"
        ][0]
        person2_id = trips_with_ids.filter(pl.col("person_id") == 200)[
            "linked_trip_id"
        ][0]

        assert person1_id != person2_id
