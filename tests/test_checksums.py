...
def overcount_seed() -> pl.DataFrame:
    """Seed where p_gender incidence sums > h_size for HH3 (double-classified)."""
    return pl.DataFrame(
        {
            "hh_id": [1, 2, 3],
            "h_size": [1, 3, 2],
...
class TestCheckIncidenceSums:
    """Tests for overcount detection on the seed table."""

    def test_passes_on_correct_seed(self, good_seed, caplog):
        """No error when all incidence sums match."""
        with caplog.at_level(logging.INFO):
            check_incidence_sums(
                good_seed,
                ["h_size", "h_workers", "p_gender", "p_employment"],
                source_label="test",
            )
        assert "all person controls pass" in caplog.text

    def test_undercount_does_not_raise(self, good_seed, caplog):
        """Undercount (< h_size) is NOT an error — caught by check_recode_nulls."""
    ...
    def test_missing_incidence_cols_skipped(self, caplog):
        ...
        seed = pl.DataFrame({"hh_id": [1], "h_size": [1], "h_workers": [0]})
        with caplog.at_level(logging.INFO):
            check_incidence_sums(seed, ["h_workers", "p_gender"], source_label="test")
        assert "all person controls pass" in caplog.text
