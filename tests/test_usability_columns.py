...
writes is a parameter, so a later change can stamp several verdicts side by
side -- one per set of admission rules -- without any of them overwriting
another.

That only holds if the parameterisation is *total*. A single ``"model_usable"``
left hardcoded anywhere in the cascade would make a second pass read the first
...
ALIAS = UsabilityProfile(name="alt_usable")
...
class TestParameterisationIsTotal:
    ...
    def test_every_table_agrees_under_either_name(self, stamped):
        ...
        disagreed = {
            name: (df[MODEL_USABLE.flag].to_list(), df[ALIAS.flag].to_list())
            for name, df in stamped.items()
            if MODEL_USABLE.flag in df.columns
            and df[MODEL_USABLE.flag].to_list() != df[ALIAS.flag].to_list()
        }
    ...
    def test_the_household_day_reduction_agrees_too(self, stamped):
        ...
        days = stamped["days"]
        assert days[MODEL_USABLE.household_day].to_list() == days[ALIAS.household_day].to_list()
...
class TestTheCascadeStillDiscriminates:
    ...
    def test_the_invalid_tour_is_rejected(self, stamped):
        ...
        assert stamped["tours"][MODEL_USABLE.flag].to_list() == [True, False]

    def test_rejection_reaches_the_member_trips(self, stamped):
        """Its linked and unlinked trips follow it out."""
        assert stamped["linked_trips"][MODEL_USABLE.flag].to_list() == [True, False]
        assert stamped["unlinked_trips"][MODEL_USABLE.flag].to_list() == [True, False]
    ...
    def test_the_joint_grouping_loses_its_quorum(self, stamped):
        ...
        assert stamped["joint_tours"][MODEL_USABLE.flag].to_list() == [False]
        assert stamped["joint_trips"][MODEL_USABLE.flag].to_list() == [False]

    def test_the_household_day_fails_for_both_members(self, stamped):
        """One member's day being unusable takes the whole date down."""
        assert stamped["days"][MODEL_USABLE.household_day].to_list() == [False, False]
