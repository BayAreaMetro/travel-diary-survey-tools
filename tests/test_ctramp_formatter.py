...
class TestHouseholdFieldCorrections:
    ...
    def test_jtf_choice_computed_from_joint_tours(self, standard_config):
        ...
        # Create 2 joint tours for household
        tours = pl.DataFrame(
            [
                create_tour(tour_id=1001, hh_id=1, person_id=101, joint_tour_id=9001),
                create_tour(tour_id=1002, hh_id=1, person_id=102, joint_tour_id=9001),
                create_tour(tour_id=1003, hh_id=1, person_id=101, joint_tour_id=9002),
                create_tour(tour_id=1004, hh_id=1, person_id=102, joint_tour_id=9002),
            ],
            schema=get_tour_schema(),
        )
        ...
        households_ctramp = result["households_ctramp"]
        # jtf_choice is based on the combination of tour purposes, not just count
        # With 2 joint tours of unspecified purpose, result depends on purpose mapping
        assert households_ctramp["jtf_choice"][0] >= 1, "Should have valid jtf_choice"
    ...
    def test_inmf_choice_binned_to_codebook(self, standard_config):
        ...
        tours = pl.DataFrame(
            [
                ...
                create_tour(tour_id=1002, person_id=102, tour_purpose=Purpose.SHOPPING_ERRANDS),
                # Person 103: 3 non-mandatory tours
                create_tour(tour_id=1003, person_id=103, tour_purpose=Purpose.SHOPPING_ERRANDS),
                create_tour(tour_id=1004, person_id=103, tour_purpose=Purpose.DINING),
                create_tour(tour_id=1005, person_id=103, tour_purpose=Purpose.SOCIAL),
            ],
            schema=get_tour_schema(),
        )
...
class TestIndividualTourFieldCorrections:
        ...
        tours = pl.DataFrame(
            [
                ...
                create_tour(
                    tour_id=1002,
                    person_id=101,
                    tour_purpose=Purpose.SHOPPING_ERRANDS,
                ),
                # At-work subtour
                create_tour(
                    tour_id=1003,
    ...
    def test_tour_purpose_not_all_othdisc(self, standard_config):
        ...
        tours = pl.DataFrame(
            [
                create_tour(tour_id=1001, person_id=101, tour_purpose=Purpose.PRIMARY_WORKPLACE),
                create_tour(tour_id=1002, person_id=101, tour_purpose=Purpose.SCHOOL),
                create_tour(tour_id=1003, person_id=101, tour_purpose=Purpose.SHOPPING_ERRANDS),
                create_tour(tour_id=1004, person_id=101, tour_purpose=Purpose.DINING),
            ],
            schema=get_tour_schema(),
        )
        trips = pl.DataFrame(
