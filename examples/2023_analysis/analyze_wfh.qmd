...
EMPLOYED_CODES = [1, 2, 3]  # Fulltime, parttime, self-employed
...
| 2-3 days per week      | `telework_freq` in \[4, 5\]                       | `commute_freq` in \[4, 5\]                        |
| 1 day per week         | `telework_freq` == 6                              | `commute_freq` == 6                               |
| Less than once a week  | `telework_freq` in \[7, 8\]                       | `commute_freq` in \[7, 8\]                        |
| Never                  | `telework_freq` == 996                            | `commute_freq` == 996 OR `job_type` == 3          |
...
|-----------------------------------|-------------------------------------------------------------------------------------------------------|
| Never, or less than once a week   | `telework_freq` in [996, 995, 7, 8] AND `job_type` != 3                                               |
...
| Always                            | (`telework_freq` in [1, 2] AND `commute_freq` == 996) OR `job_type` == 3                              |
...
telework_conditions = {
    ...
    # Works 2-3 days
    FrequencyCategory.TWO_THREE: (pl.col("telework_freq").is_in([4, 5])),
    # Works 1 day
    FrequencyCategory.ONE: (pl.col("telework_freq") == 6),
    # Less than weekly
    FrequencyCategory.LESS_THAN_WEEKLY: (pl.col("telework_freq").is_in([7, 8])),
    # Never
    FrequencyCategory.NEVER: (pl.col("telework_freq") == 996),
    # Missing
...
commute_conditions = {
    ...
    # Works 2-3 days
    FrequencyCategory.TWO_THREE: (pl.col("commute_freq").is_in([4, 5])),
    # Works 1 day
    FrequencyCategory.ONE: (pl.col("commute_freq") == 6),
    # Less than weekly
    FrequencyCategory.LESS_THAN_WEEKLY: (pl.col("commute_freq").is_in([7, 8])),
    # Never
...
fig.update_yaxes(title_text="% (Weighted)")
