...
def write_summary_report(
    ...
    if "households" in input_summaries and "households" in standard_summaries:
        ...
        report_lines.append(f"Households: {format_number(hh_in)} → {format_number(hh_out)} ({hh_diff:+,})")
    ...
    if "persons" in input_summaries and "persons" in standard_summaries:
        ...
        report_lines.append(f"Persons:    {format_number(per_in)} → {format_number(per_out)} ({per_diff:+,})")
    ...
    if "days" in input_summaries and "days" in standard_summaries:
        ...
        report_lines.append(f"Days:       {format_number(day_in)} → {format_number(day_out)} ({day_diff:+,})")
    ...
    if "trips" in input_summaries and "unlinked_trips" in standard_summaries:
        ...
        report_lines.append(f"Trips:      {format_number(trip_in)} → {format_number(trip_out)} unlinked ({trip_diff:+,})")
    ...
    if "linked_trips" in standard_summaries:
        report_lines.append(f"  Linked Trips: {format_number(standard_summaries['linked_trips']['rows'])}")
    if "tours" in standard_summaries:
        report_lines.append(f"  Tours:        {format_number(standard_summaries['tours']['rows'])}")
    if "joint_trips" in standard_summaries:
        report_lines.append(f"  Joint Trips:  {format_number(standard_summaries['joint_trips']['rows'])}")
    ...
    if "tours" in standard_summaries:
        ...
        if "tour_data_quality" in tours_df.columns:
            ...
            quality_counts = tours_df.group_by("tour_data_quality").agg(pl.len()).sort("tour_data_quality")
            for row in quality_counts.iter_rows():
                ...
                report_lines.append(f"    {quality_label:60s}: {format_number(count):>8s} ({pct:5.1f}%)")
            report_lines.append("")
        ...
        if "tour_category" in tours_df.columns:
            ...
            for row in category_counts.iter_rows():
                ...
                pct = (count / len(tours_df) * 100)
                report_lines.append(f"    {category_label:60s}: {format_number(count):>8s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "tours" in standard_summaries and "tours_daysim" in daysim_summaries:
        ...
        report_lines.append(f"  Total Tours Dropped: {format_number(tours_dropped)} of {format_number(tours_std)} ({pct_dropped:.1f}%)")
    ...
    if "linked_trips" in standard_summaries and "linked_trips_daysim" in daysim_summaries:
        ...
        report_lines.append(f"  Total Linked Trips Dropped: {format_number(trips_dropped)} of {format_number(trips_std)} ({pct_dropped:.1f}%)")
    ...
    if "trips" in input_summaries and "linked_trips" in standard_summaries:
        ...
        if new_cols:
            ...
            for col in new_cols[:20]:  # Limit to first 20
                report_lines.append(f"  • {col}")
            if len(new_cols) > 20:
                report_lines.append(f"  ... and {len(new_cols) - 20} more")
            report_lines.append("")
    ...
    if "tours" in standard_summaries:
        ...
        if "tour_purpose" in tours_df.columns:
            ...
            for row in purpose_counts.iter_rows():
                purpose, count = row
                pct = (count / len(tours_df) * 100)
                report_lines.append(f"  {str(purpose):20s}: {format_number(count):>10s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "linked_trips" in standard_summaries:
        ...
        if "mode" in trips_df.columns:
            ...
            for row in mode_counts.iter_rows():
                ...
                report_lines.append(f"  {str(mode):20s}: {format_number(count):>10s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "tours" in standard_summaries:
        ...
        if "tour_category" in tours_df.columns:
            ...
            for row in person_counts.iter_rows():
                category, count = row
                pct = (count / len(tours_df) * 100)
                report_lines.append(f"  {str(category):20s}: {format_number(count):>10s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "households" in standard_summaries:
        ...
        if "num_persons" in hh_df.columns:
            ...
            for row in size_counts.iter_rows():
                ...
                report_lines.append(f"  {size} person(s): {format_number(count):>10s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "persons" in standard_summaries:
        ...
        if "age" in persons_df.columns:
            ...
            for row in age_counts.iter_rows():
                ...
                report_lines.append(f"  {age_label:20s}: {format_number(count):>10s} ({pct:5.1f}%)")
            report_lines.append("")
    ...
    if "joint_trips" in standard_summaries:
        ...
        if "linked_trip_id_1" in joint_df.columns:
            unique_trips = len(set(joint_df["linked_trip_id_1"].to_list() + joint_df["linked_trip_id_2"].to_list()))
    ...
    if "tours" in standard_summaries:
        ...
        if "person_id" in tours_df.columns:
            ...
            tour_dist = tours_per_person.group_by("num_tours").agg(pl.len().alias("num_persons")).sort("num_tours")
            for row in tour_dist.iter_rows():
                num_tours, num_persons = row
                pct = (num_persons / len(tours_per_person) * 100)
                report_lines.append(f"  {num_tours} tour(s): {format_number(num_persons):>10s} persons ({pct:5.1f}%)")
            report_lines.append("")
...
if __name__ == "__main__":
    # Default paths
    INPUT_DIR = Path("M:/Data/HomeInterview/Bay Area Travel Study 2023/Data/Full Weighted 2023 Dataset/WeightedDataset_02212025")
    OUTPUT_DIR = Path("C:/bats_processing/outputs")
